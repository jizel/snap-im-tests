...
function extract_usage_2555417730620(request)
    ...
    if (m and (method == "GET" or method == "POST" or method == "DELETE") ) then
        -- rule: / --
...
function get_auth_params(where, method)
    ...
    elseif method == "GET" or method == "POST" or method == "DELETE" then
...
function get_debug_value()
    ...
    if h["X-3scale-debug"] == ngx.var.provider_key then
        return true
    else
        return false
    end
...
function oauth(params, service)
	...
	local exists, hasie = redis.token_exists_in_cache(params.access_token)

	if exists == false then 
            access_tokens:delete(ngx.var.cached_key)
            ngx.status = ngx.HTTP_FORBIDDEN
            ngx.header.content_type = "application/json"
		ngx.log(ngx.ERR, "[DEBUG] tutaj jestem!!!!!!")
...
function M.verify_token()
    ...
    local auth_strat = ""
    local service = {}
    if ngx.var.service_id == '2555417730620' then
        local parameters = get_auth_params("GET", string.split(ngx.var.request, " ")[1] )
        service = service_2555417730620 --
        ngx.var.secret_token = service.secret_token

        -- Get access token from response
        params.access_token = parameters["access_token"]
        ngx.var.access_token = params.access_token
        get_credentials_access_token(params, service_2555417730620)
        ngx.var.cached_key = "2555417730620" .. ":" .. params.access_token
        auth_strat = "oauth"
        ngx.var.service_id = "2555417730620"
        ngx.var.proxy_pass = "http://tomcat_backend"
        ngx.var.usage = extract_usage_2555417730620(ngx.var.request)
    end
