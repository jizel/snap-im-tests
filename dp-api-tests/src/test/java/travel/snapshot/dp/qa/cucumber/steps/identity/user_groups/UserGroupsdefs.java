package travel.snapshot.dp.qa.cucumber.steps.identity.user_groups;
...
public class UserGroupsdefs {
    ...
    //    Help methods
    private Map<String, UUID> getNonNullIdsFromNames(String userGroupName, String userName) {
        UUID userId = usersSteps.resolveUserId(userName);
        UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);

        Map<String, UUID> userGroupUserIds = new HashMap<>();
...
//    End of help method section
    ...
    public void userGroupIsRequestedByUser(String userGroupName, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
    ...
    public void userGroupWithIdIsActivated(UUID userGroupId) throws Throwable {
        userGroupSteps.setUserGroupActiveField(userGroupId, true);
    }

    @When("^User group with id \"([^\"]*)\" is deactivated$")
    public void userGroupWithIdIsDeactivated(UUID userGroupId) throws Throwable {
    ...
    public void userGroupWithIdIsUpdatedWithFollowingData(UUID userGroupId, List<UserGroupUpdateDto> userGroups) throws Throwable {
    ...
    public void relationBetweenUserGroupAndRoleIsCreatedByUser(String userGroupName, UUID roleId, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
    ...
    public void relationBetweenUserGroupAndRoleExists(String userGroupName, UUID roleId, String isActiveString) throws Throwable {
        Boolean isActive = ((isActiveString==null) ? true : Boolean.valueOf(isActiveString));
        UUID userGroupId= userGroupSteps.resolveUserGroupId(userGroupName);
        userGroupSteps.relationshipGroupRoleExist(userGroupId, roleId, isActive);
    }

    @When("^Relation between user group \"([^\"]*)\" and property(?: with code)? \"([^\"]*)\" is created(?: with isActive \"([^\"]*)\")?(?: by user \"([^\"]*)\")?(?: for application version \"([^\"]*)\")?(?: with error \"([^\"]*)\")?$")
    public void relationBetweenUserGroupAndPropertyWithCodeIsCreatedWithIsActiveByUser(String userGroupName, String propertyCode, Boolean isActive, String username, String applicationVersionName, Boolean error) throws Throwable {
        UUID userId = usersSteps.resolveUserId(username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
        if (error){
            userGroupSteps.createUserGroupPropertyRelationshipByUserForApp(userId, applicationVersionId, userGroupSteps.resolveUserGroupId(userGroupName), propertySteps.resolvePropertyId(propertyCode), isActive);
        }
        else {
            UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);
            UUID propertyId = propertySteps.resolvePropertyId(propertyCode);
            userGroupSteps.createUserGroupPropertyRelationshipByUserForAppInvalid(userId, applicationVersionId, userGroupId, propertyId, isActive);
        }
    }

    @When("^Relation between user group \"([^\"]*)\" and property set(?: with id)? \"([^\"]*)\" is created(?: with isActive \"([^\"]*)\")?(?: by user \"([^\"]*)\")?(?: for application version \"([^\"]*)\")?(?: with error \"([^\"]*)\")?$")
    public void relationBetweenUserGroupAndPropertySetIsCreatedWithIsActiveByUser(String userGroupName, String propertySetName, Boolean isActive, String username, String applicationVersionName, Boolean error) throws Throwable {
        UUID userId = usersSteps.resolveUserId(username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);

        if (error){
            userGroupSteps.userGroupPropertySetRelationshipIsCreatedByUserForApp(userId, applicationVersionId, userGroupSteps.resolveUserGroupId(userGroupName), propertySetSteps.resolvePropertySetId(propertySetName), isActive);
        }
        else {
            UUID propertySetId = propertySetSteps.resolvePropertySetId(propertySetName);
            UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);
            userGroupSteps.userGroupPropertySetRelationshipIsCreatedByUserForApp(userId, applicationVersionId, userGroupId, propertySetId, isActive);
        }
    }

    @When("^Relation between user group \"([^\"]*)\" and property(?: with code)? \"([^\"]*)\" is deleted(?: by user \"([^\"]*)\")?(?: for application version \"([^\"]*)\")?(?: with error \"([^\"]*)\")?$")
    public void relationBetweenUserGroupAndPropertyWithCodeIsDeletedIsDeletedByUser(String userGroupName, String propertyCode, String username, String applicationVersionName, Boolean error) throws Throwable {
        UUID userId = usersSteps.resolveUserId(username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);

        if(error){
            userGroupSteps.deleteUserGroupPropertyRelationshipByUserForApp(userId, applicationVersionId, userGroupSteps.resolveUserGroupId(userGroupName), propertySteps.resolvePropertyId(propertyCode));
        }
        else{
            UUID propertyId = propertySteps.resolvePropertyId(propertyCode);
            UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);
            userGroupSteps.deleteUserGroupPropertyRelationshipByUserForApp(userId, applicationVersionId, userGroupId, propertyId);
        }
    }

    @When("^Relation between user group(?: with id)? \"([^\"]*)\" and property set \"([^\"]*)\" is deleted(?: by user \"([^\"]*)\")?(?: for application version \"([^\"]*)\")?(?: with error \"([^\"]*)\")?$")
    public void relationBetweenUserGroupAndPropertySetIsDeletedByUser(String userGroupName, String propertySetName, String username, String applicationVersionName, Boolean error) throws Throwable {
        UUID userId = usersSteps.resolveUserId(username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);

        if(error) {
            userGroupSteps.relationshipGroupPropertySetIsDeletedByUserForApp(userId, applicationVersionId, userGroupSteps.resolveUserGroupId(userGroupName), propertySetSteps.resolvePropertySetId(propertySetName));
        }
        else{
            UUID propertySetId = propertySetSteps.resolvePropertySetId(propertySetName);
            UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);
            userGroupSteps.relationshipGroupPropertySetIsDeletedByUserForApp(userId, applicationVersionId, userGroupId, propertySetId);
        }
    }

    @When("^Relation between user group \"([^\"]*)\" and role \"([^\"]*)\" is deleted$")
    public void relationBetweenUserGroupAndRoleIsDeleted(String userGroupName, UUID roleId) throws Throwable {
        UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);
    ...
    public void relationBetweenUserGroupAndRoleIsDeletedByUser(String userGroupName, UUID roleId, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
    ...
    public void relationBetweenUserGroupAndPropertyIsActivated(String userGroupName, String propertyCode, String negation) throws Throwable {
        UUID groupId = userGroupSteps.resolveUserGroupId(userGroupName);
        UUID propertyId = propertySteps.resolvePropertyId(propertyCode);
    ...
    public void userGroupWithIdIsActive(UUID userGroupId) throws Throwable {
        userGroupSteps.checkUserGroupActiveField(userGroupId, true);
    }

    @Then("^User group with id \"([^\"]*)\" is not active$")
    public void userGroupWithIdIsNotActive(UUID userGroupId) throws Throwable {
        userGroupSteps.checkUserGroupActiveField(userGroupId, false);
    }

    @Then("^User group with id \"([^\"]*)\" exists no more$")
    public void userGroupWithIdIsNoMoreExists(UUID userGroupId) throws Throwable {
        userGroupSteps.checkUserGroupExistency(userGroupId, false);
    }

    @Then("^User group with id \"([^\"]*)\" contains following data$")
    public void userGroupWithIdContainsFollowingData(UUID userGroupId, List<UserGroupUpdateDto> userGroups) throws Throwable {
    ...
    public void relationBetweenUserGroupAndPropertyIsNoMoreExists(UUID userGroupId, UUID propertyId) throws Throwable {
    ...
    public void relationBetweenUserGroupAndRoleIsEstablished(String userGroupName, UUID roleId) throws Throwable {
        UserGroupDto userGroup = userGroupSteps.getUserGroupByName(userGroupName);
        assertThat(userGroup, is(notNullValue()));
    ...
    public void relationBetweenUserGroupAndRoleIsNotEstablished(String userGroupName, UUID roleId) throws Throwable {
        UUID userGroupId = userGroupSteps.resolveUserGroupId(userGroupName);
    ...
    public void relationBetweenUserGroupAndUserWithIdIsCreatedWithIsActive(String userGroupName, UUID userId, Boolean isActive) throws Throwable {
        UserGroupDto userGroup = userGroupSteps.getUserGroupByName(userGroupName);
    ...
    public void relationBetweenUserGroupPropertyWithCodeAndRoleWithIdIsCreatedByUser(String userGroupName, String propertyCode, UUID roleId, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID propertyId = propertySteps.resolvePropertyId(propertyCode);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
    ...
    public void relationBetweenUserGroupPropertyWithCodeAndRoleWithIdIsUpdatedByUser(String userGroupName, String propertyCode, UUID roleId, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID propertyId = propertySteps.resolvePropertyId(propertyCode);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
    ...
    public void relationBetweenUserGroupPropertySetAndRoleWithIdIsCreatedByUser(String userGroupName, String propertySetName, UUID roleId, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID propertySetId = propertySetSteps.resolvePropertySetId(propertySetName);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
    ...
    public void relationBetweenUserGroupPropertySetAndRoleWithIdIsDeletedByUser(String userGroupName, String propertySetName, UUID roleId, String username, String applicationVersionName) throws Throwable {
        Map<String, UUID> ids = getNonNullIdsFromNames(userGroupName, username);
        UUID propertySetId = propertySetSteps.resolvePropertySetId(propertySetName);
        UUID applicationVersionId = applicationVersionSteps.resolveApplicationVersionId(applicationVersionName);
