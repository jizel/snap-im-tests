Feature: Eventing_configuration_module
  ...
  Scenario: Eventing configurationType created
    ...
    Given Subscription with name "configuration_eventing" for topic "Notifications" does not exist
    Given Subscription with name "configuration_eventing" for topic "Notifications" is created
    When Configuration type is created
      | identifier    | description                                       |
      | eventing_test | Description of created configuration identifier 1 |
    Then Message is received with subscription "configuration_eventing" from topic "Notifications" and stored in session
    ...
    And Subscription with name "configuration_eventing" for topic "Notifications" is unsubscribed
