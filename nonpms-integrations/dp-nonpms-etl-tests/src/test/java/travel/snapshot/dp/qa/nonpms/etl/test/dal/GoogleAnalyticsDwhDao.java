...
public interface GoogleAnalyticsDwhDao extends IntegrationDwhDao {
    ...
    @Select("SELECT * FROM Fact_web_performance_cumulative f WHERE f.date_id = #{dateId} AND f.property_id = " +
    ...
    @Delete("DELETE FROM Fact_web_performance_cumulative WHERE date_id >= #{since} AND date_id <= #{until} AND property_id = " +
            "( SELECT property_id FROM Dim_property WHERE property_key = #{propertyKey} )")
    void deleteData(@Param("propertyKey") String propertyId, @Param("since") Integer since, @Param("until") Integer until);

    @Insert("REPLACE INTO Fact_web_performance_cumulative (property_id, date_id, users, visits, transactions, revenue) " +
