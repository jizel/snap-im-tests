Feature: twitter_tweets
  Testing of api for twitter tweets with mock data in db - testing property id is "99999999-9999-4999-a999-999999999999"

  #not yet implemented
  Scenario Outline: Getting a list of items
    When List of "<url>" is got with limit "<limit>" and cursor "<cursor>"
    Then Response code is "200"
    And Content type is "application/json"
    And Data is owned by "twitter"
    And There are <count> posts returned

    Examples: 
      | url                                    | limit | cursor | count |
      | /social_media/analytics/twitter/tweets |       |        | 50    |
      | /social_media/analytics/twitter/tweets | /null |        | 50    |
      | /social_media/analytics/twitter/tweets |       | /null  | 50    |
      | /social_media/analytics/twitter/tweets | /null | /null  | 50    |
      | /social_media/analytics/twitter/tweets | 51    |        | 50    |
      | /social_media/analytics/twitter/tweets | 51    | /null  | 50    |
      | /social_media/analytics/twitter/tweets |       | 1      | 50    |
      | /social_media/analytics/twitter/tweets | /null | 1      | 50    |
      | /social_media/analytics/twitter/tweets | 20    | 0      | 20    |
      | /social_media/analytics/twitter/tweets | 60    | 0      | 50    |
      | /social_media/analytics/twitter/tweets | 5     | 5      | 5     |

  #not yet implemented, other metrics are not pageable
  Scenario Outline: Checking error codes for getting list of items
    When List of "<url>" is got with limit "<limit>" and cursor "<cursor>"
    Then Response code is "<response_code>"
    And Custom code is "<custom_code>"

    Examples: 
      | url                                    | limit | cursor | response_code | custom_code |
      | /social_media/analytics/twitter/tweets | /null | -1     | 400           | 52          |
      | /social_media/analytics/twitter/tweets |       | -1     | 400           | 52          |
      | /social_media/analytics/twitter/tweets | 10    | -1     | 400           | 63          |
      | /social_media/analytics/twitter/tweets | text  | 0      | 400           | 63          |
      | /social_media/analytics/twitter/tweets | text  | /null  | 400           | 63          |
      | /social_media/analytics/twitter/tweets | text  |        | 400           | 63          |
      | /social_media/analytics/twitter/tweets | 10    | text   | 400           | 63          |
      
      #To Do: sorting tests should be designed.
