 ...
 * 
 * Midnight ETLs occur at 00:01 local time for hotels in time zones [-6;+4]. Tests are meant to be run after all ETLs have passed,
 * and before noon ETLs for the next day have started.
*/
...
public class TestFacebookPagesMidnightETL {
    ...
    public void testStagingLoad() throws Exception {
    	//Midnight ETL for each hotel is triggered after the first run to get data from Facebook API has passed.
    	//This ETL isn't triggered on the very first run
    	//The check for counts should pass after all ETLs for the past day are finished
        ...
        followUpListToSource.add("select sum(total_followers) from RawImportedFacebookPageStatistics where date = curdate() and data_collection_run = 1");
    ...
    public void testFactLoad() throws Exception {
        ...
        String sqlQueryForTarget = "select count(*)*2 from FactFacebookPageStats where dim_date_id = (curdate() - interval 1 day) + 0";
        ...
        factsTodayList.add("select sum(number_of_posts) from FactFacebookPageStats where dim_date_id = (curdate() - interval 1 day) + 0");
        ...
        factsTodayList.add("select sum(reach) from FactFacebookPageStats where dim_date_id = (curdate() - interval 1 day) + 0");
